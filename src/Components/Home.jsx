...
import { Search, More } from '@mui/icons-material';
...
const Home = () => {
    ...
    return (
        <>
           <Box style={{backgroundColor:'#EAF3FF'}}>
            <SearchBar style={{ border: '1px solid black', width: '200px', marginLeft: '90px'}}>
                <SearchIconWrapper>
                    <Search />
                </SearchIconWrapper>
                <StyledInputBase
            ...
            <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', marginLeft: '70px' }}>
                {rows?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                    .map(({ title, date, avtar }) => (
                        <>
                            <Card sx={{ maxWidth: 270, margin: 3, height: 260 }}>
                                <CardHeader
                                    ...
                                    action={
                                        <>
                                            ...
                                            >
                                                {options.map((option) => (
                                                    <MenuItem key={option} selected={option === 'Edit' || 'delete'} onClick={handleClose}>
                                                        {option}
                                                    </MenuItem>
                                                ))}
                                            </Menu></>}
            ...
            <TablePagination sx={{ mr: 15, position: 'relative' ,top:'100'}}
                component="div"
                count={Data?.length}
                page={page}
